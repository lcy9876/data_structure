...
public:
    ...
    //快速删除
    iterator erase_fast(iterator from, iterator to)
    {
       if (from == to) {
            return to; // 如果区间为空，则直接返回
        }

        // 保存要删除的第一个和最后一个节点的前一个节点
        Node* first = from.current->prev;
        Node* last = to.current->prev;
        ...
        return iterator(last); // 返回指向区间结束后第一个节点的迭代器
    }
